package com.iqmsoft.spring.integration.shop.flow;
...
public class OrderFlows {
	...
	public IntegrationFlow orderFlow() {
		...
		return from(messageDriverChannelAdapter(connectionFactory).destination(orderQueue()))
				.filter("@orderValidator.validate(payload)").split("payload.items", (Consumer) null)
				.filter("@warehouseCheckService.isAvailable(payload)")
				.routeToRecipients(r -> r
						.recipientFlow("payload.isSimCard()",
								subFlow -> subFlow.handle("discountService", "addSimDiscount")
										.channel("confirmation.input"))
						.recipientFlow("payload.isSmartphone()",
								subFlow -> subFlow.handle("discountService", "addPhoneDiscount")
										.channel("confirmation.input"))
						.recipientFlow("payload.isTablet()",
								subFlow -> subFlow.handle("discountService", "addTabletDiscount")
										.channel("confirmation.input"))
						.recipientFlow("payload.isMPlayer()", subFlow -> subFlow
								.handle("discountService", "addMPlayerDiscount").channel("confirmation.input")))
				.get();
	}

	@Bean
	public IntegrationFlow confirmation() {
		return f -> f.aggregate().handle("summaryService", "sumUp")
